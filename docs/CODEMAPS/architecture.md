...
| 测试 | pytest + pytest-asyncio (41 测试文件, ~11300 行) |
| 代码行数 | ~11721 行核心代码（42 模块） + ~11300 行测试 |
...
+---------------------------------------------------------+
|                编排层 (Orchestration)                     |
|   Orchestrator (1810行, 两阶段进化+Metric校验+sanitize)   |
|   - run_epoch_draft(): Phase 1 初稿生成                   |
|   - run_epoch(): Phase 2 GA 进化                         |
|   - _sanitize_metric_value(): 入口校验                    |
+---------------------------------------------------------+
|                  Agent 层 (Agents)                        |
|   BaseAgent (144行) + CoderAgent (538行, +draft任务)     |
|   PromptManager (288行, Jinja2 模板)                     |
+---------------------------------------------------------+
|                进化层 (Evolution)                         |
|   GeneParser (201行, 4基因+sub-aspects)                  |
|   GeneCompatibility (141行, 框架互斥检测)                 |
|   ExperiencePool (319行)                                 |
|   TaskDispatcher (157行)                                 |
|   AgentEvolution (437行)                                 |
|   GeneRegistry (196行)                                   |
|   GeneSelector (463行, +退化检测+lower_is_better)        |
|   Pheromone (108行, +lower_is_better方向排序)            |
|   SolutionEvolution (351行, 两阶段GA+方向感知)           |
|   CodeEmbeddingManager (130行)                           |
|   SkillExtractor (302行)                                 |
|   SkillManager (429行)                                   |
+---------------------------------------------------------+
|                  执行层 (Execution)                       |
|   Interpreter (440行, 精简重构+并行增强)                   |
...
|   config.py (545行) + logger_system.py (180行)           |
|   file_utils.py (222行) + text_utils.py (144行)          |
|   system_info.py (444行) + data_preview.py (390行)       |
|   proxy.py (205行) + metric.py (117行)                   |
...
graph TD
    subgraph "基础设施"
        CFG[utils/config.py<br/>配置管理 545行]
        LOG[utils/logger_system.py<br/>日志系统 180行]
        FU[utils/file_utils.py<br/>文件工具 222行]
        TU[utils/text_utils.py<br/>文本压缩 144行]
        SYSINFO[utils/system_info.py<br/>系统信息 444行]
        YAML[config/default.yaml<br/>YAML 配置]
        ENV[.env<br/>环境变量]
    end

    subgraph "数据结构"
        NODE[core/state/node.py<br/>Node 数据类 105行<br/>+两阶段字段]
    ...
    subgraph "执行层"
        INTERP[core/executor/interpreter.py<br/>代码执行沙箱 440行]
        WS[core/executor/workspace.py<br/>工作空间管理 280行]
    end

    subgraph "Agent 层"
        AGENT[agents/base_agent.py<br/>BaseAgent 144行<br/>+draft task_type]
        CODER[agents/coder_agent.py<br/>CoderAgent 538行<br/>+draft/merge/mutate]
        PM[utils/prompt_manager.py<br/>PromptManager 288行]
    end

    subgraph "编排层"
        ORCH[core/orchestrator.py<br/>任务编排器 1810行<br/>两阶段进化+纯文本Review]
    end

    subgraph "进化层"
        GENE[core/evolution/gene_parser.py<br/>基因解析器 201行<br/>4基因+sub-aspects]
        GCOMPAT[core/evolution/gene_compatibility.py<br/>基因兼容性 141行]
        ...
        GSEL[core/evolution/gene_selector.py<br/>基因选择器 463行]
        PHER[core/evolution/pheromone.py<br/>信息素机制 108行]
        SEVO[core/evolution/solution_evolution.py<br/>Solution 层 GA 351行]
...
| **1** | 核心数据结构 | **完成** | Node (105行), Journal (372行), Task (62行) |
| **1** | 后端抽象层 | **完成** | Backend + query_with_config + key_pool |
| **2** | 执行层 | **完成** | Interpreter (440行), WorkspaceManager (280行) |
| **2** | Agent 抽象 | **完成** | BaseAgent (144行) + draft task_type |
| **2** | CoderAgent | **完成** | CoderAgent (538行, +draft/merge/mutate) |
| **2.4** | Orchestrator | **完成** | Orchestrator (1810行, 两阶段进化+纯文本Review) |
| **3.1** | 基因解析器 | **完成** | gene_parser.py (201行, 4基因+sub-aspects) |
| **3.1** | 基因兼容性 | **完成** | gene_compatibility.py (141行) |
| **3.2** | 经验池 | **完成** | experience_pool.py (319行) |
| **3+** | PromptManager | **完成** | prompt_manager.py (288行) + benchmark/ |
| **3.3** | Agent 层群体智能 | **完成** | task_dispatcher.py (157行) + agent_evolution.py (437行) |
| **3.4** | Solution 层 GA | **完成** | solution_evolution.py (351行) + gene_selector.py (463行, +退化检测+lower_is_better) + gene_registry.py (196行) + pheromone.py (108行) |
...
|------|------|------|------|
| **入口层** ||||
| 双层进化入口 | `main.py` | 646 | 完成 (两阶段架构) |
| 评测适配器 | `run_mle_adapter.py` | 379 | 完成 |
| **Phase 1: 基础设施** ||||
| 配置管理 | `utils/config.py` | 545 | 完成 (+两阶段EvolutionConfig) |
| 日志系统 | `utils/logger_system.py` | 180 | 完成 |
| 文件工具 | `utils/file_utils.py` | 222 | 完成 |
| 文本压缩工具 | `utils/text_utils.py` | 144 | 完成 (+Review压缩+截断策略) |
| 系统信息 | `utils/system_info.py` | 444 | 完成 |
| 数据预览 | `utils/data_preview.py` | 390 | 完成 |
| 代理设置 | `utils/proxy.py` | 205 | 完成 |
...
| **Phase 1: 数据结构** ||||
| Node 数据类 | `core/state/node.py` | 105 | 完成 (+两阶段字段) |
...
| **Phase 2: 执行层** ||||
| 代码执行器 | `core/executor/interpreter.py` | 440 | 完成 |
| 工作空间管理 | `core/executor/workspace.py` | 280 | 完成 |
| **Phase 2: Agent 层** ||||
| Agent 基类 | `agents/base_agent.py` | 144 | 完成 (+draft) |
| CoderAgent | `agents/coder_agent.py` | 538 | 完成 (+draft/merge/mutate) |
| **Phase 2.4: Orchestrator** ||||
| 任务编排器 | `core/orchestrator.py` | 1810 | 完成 (两阶段进化+纯文本Review+sanitize) |
| **Phase 3: 进化层** ||||
| 基因解析器 | `core/evolution/gene_parser.py` | 201 | 完成 (4基因+sub-aspects) |
| 基因兼容性 | `core/evolution/gene_compatibility.py` | 141 | 完成 |
...
| 基因选择器 | `core/evolution/gene_selector.py` | 463 | 完成 (+退化检测+lower_is_better) |
| 信息素机制 | `core/evolution/pheromone.py` | 108 | 完成 (+lower_is_better方向排序) |
| Solution 层 GA | `core/evolution/solution_evolution.py` | 351 | 完成 (两阶段GA+方向感知) |
| **Phase 3.5: Skill 进化** ||||
...
| **Phase 3+: Prompt 系统** ||||
| Prompt 管理器 | `utils/prompt_manager.py` | 288 | 完成 |
| Benchmark 资源 | `benchmark/mle-bench/` | - | 完成 |
| **配置文件** ||||
| YAML 配置 | `config/default.yaml` | ~131 | 完成 (+两阶段进化配置) |
| MLE-Bench 配置 | `config/mle_bench.yaml` | 29 | 完成 (评测专用覆盖) |

**总计**: 42 个核心模块 | ~11721 行核心代码 + 41 个测试文件（~11300 行测试代码）
...
├── config/
│   ├── default.yaml               # 统一 YAML 配置 (~131行)
│   └── mle_bench.yaml             # MLE-Bench 评测配置 (29行)
...
├── agents/
│   ├── __init__.py
│   ├── base_agent.py              # Agent 抽象基类 (144行, +draft)
│   └── coder_agent.py             # 代码生成 Agent (538行)
├── core/
│   ├── state/
│   │   ├── node.py                # 解决方案节点 (105行, +两阶段字段)
│   │   ├── journal.py             # 解决方案日志 (372行)
│   │   └── task.py                # 任务定义 (62行)
│   ├── backend/
│   │   ├── __init__.py            # 统一查询接口 (168行)
│   │   ├── backend_openai.py      # OpenAI + GLM (163行)
│   │   ├── backend_anthropic.py   # Anthropic (153行)
│   │   ├── key_pool.py            # API Key 池 (91行)
│   │   └── utils.py               # 消息格式 + 重试 (80行)
│   ├── executor/
│   │   ├── interpreter.py         # 执行沙箱 (440行)
│   │   └── workspace.py           # 工作空间管理 (280行)
│   ├── orchestrator.py            # 编排器 (1810行, 两阶段进化+纯文本Review)
│   └── evolution/
│       ├── gene_parser.py         # 基因解析器 (201行, 4基因)
│       ├── gene_compatibility.py  # 基因兼容性 (141行)
│       ├── experience_pool.py     # 共享经验池 (319行)
│       ├── task_dispatcher.py     # 任务分配器 (157行)
│       ├── agent_evolution.py     # Agent 层进化 (437行)
│       ├── gene_registry.py       # 基因注册表 (196行)
│       ├── gene_selector.py       # 基因选择器 (463行, +退化检测+lower_is_better)
│       ├── pheromone.py           # 信息素机制 (108行, +lower_is_better)
│       ├── solution_evolution.py  # Solution 层 GA (351行, +方向感知)
...
│   ├── text_utils.py              # 文本压缩+截断 (144行)
│   ├── data_preview.py            # 数据预览 (390行)
│   ├── metric.py                  # 评估指标工具 (117行)
│   ├── response.py                # LLM 响应解析 (154行)
│   ├── prompt_manager.py          # Prompt 管理器 (288行)
│   ├── system_info.py             # 系统信息收集 (444行)
│   ├── proxy.py                   # HTTP/HTTPS 代理 (205行)
│   ├── code_validator.py          # 代码静态验证 (79行)
│   ├── submission_validator.py    # 提交格式验证 (72行)
│   └── workspace_builder.py       # 工作空间构建器 (134行)
├── tests/
│   ├── unit/                      # 单元测试 (27 个测试文件)
│   ├── test_evolution/            # 进化模块测试 (13 个测试文件)
│   └── integration/               # 集成测试 (1 个测试文件)
└── docs/
    ├── CODEMAPS/                   # 架构图
    └── plans/                     # Phase 详细计划
```
...
### 6.1 基因解析器 (`core/evolution/gene_parser.py`, 201行)
...
### 6.7 基因选择器 (`core/evolution/gene_selector.py`, 463行)

**职责**: 信息素驱动的基因选择 + 退化检测 + 主父代推断 + lower_is_better 方向排序。
...
### 6.8 信息素机制 (`core/evolution/pheromone.py`, 108行)

节点级信息素: `pheromone = w1×score + w2×success_rate + w3×time_decay`

### 6.9 Solution 层进化器 (`core/evolution/solution_evolution.py`, 351行)
